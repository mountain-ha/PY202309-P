...
def simplify_pos_terms(pos_terms):
    num_of_variables = len(pos_terms[0])
    simplified_terms = []
    for term1 in range(0, num_of_variables):
        for term2 in range(term1, num_of_variables):
            if term1 == term2: break
            elif (set(term1)-set(term2) | (set(term2)-set(term1)) == {"A","A'"} or len(set(term1)|set(term2)) == {"B","B'"} or len(set(term1))|set(term2)) == {"C","C'"} or len(set(term1)|set(term2)) == {"D","D'"}:
                simplified_terms.append(set(term1)&set(term2))
...
if karnaugh_map:
    ...
    print("최적화된 부울 함수 (POS):", simplify_pos_terms(get_pos_terms(minimized_terms_sop)))
